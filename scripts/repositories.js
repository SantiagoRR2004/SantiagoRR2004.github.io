  ...
  Object.keys(customRepos).forEach((repoUrl) => {
    ...
    if (languages && Object.keys(languages).length > 0) {
      ...
      if (languagesColors[primaryLanguage]) {
        cellLanguage.style.backgroundColor = languagesColors[primaryLanguage];
      }
    } else {
    ...
    let totalSize = 0;
    for (const size of Object.values(customRepos[repoUrl]["languages"] || {})) {
      totalSize += size;
    }
    ...
    const issuePercentage =
      customRepos[repoUrl]["userIssues"] / customRepos[repoUrl]["issues"] || 0;
